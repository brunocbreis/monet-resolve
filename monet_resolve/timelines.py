"""Timelines: find, map, back up, refresh, park the playhead."""
import time
from typing import Dict

from ._util import find_timeline, items, save  # noqa: F401 (find_timeline re-exported)
...
def map_timeline(timeline) -> Dict:
    ...
    Starts are frames relative to the timeline start. Worked 2026-09-11.
    """
...
def backup_timeline(resolve, project, timeline, backup_name: str, timeline_bin: str = "timelines",
    ...
    This is the only undo the API offers: run it before any delete-and-re-append step. Worked 2026-09-11.
    """
...
def place_playhead(resolve, project, timeline, timecode: str) -> Dict:
    ...
    Returns {"timeline": name, "tc": the timecode read back}. Worked 2026-09-11.
    """
    project.SetCurrentTimeline(timeline)
...
def refresh_timeline(project, timeline, other, pause: float = 1.0) -> None:
    ...
    retries, lock variants, and page switches did not fix it; switching timelines does. Worked 2026-09-11.
    """
